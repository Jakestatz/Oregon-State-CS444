...
mem_init(void)
{
	...
	// Your code goes here:
	boot_map_region(kern_pgdir, KSTACKTOP - KSTKSIZE, KSTKSIZE, PADDR(bootstack), (PTE_U | PTE_P));
...
page_init(void)
{
	...
	size_t i;
	for (i = 0; i < npages; i++) {
		if ((i == 0) || ((i * PGSIZE) >= IOPHYSMEM && (i * PGSIZE) < EXTPHYSMEM) || ((i * PGSIZE) >= EXTPHYSMEM && (i * PGSIZE) < PADDR((char*)end)) || ((i * PGSIZE) >= PADDR((char*)end) && (i * PGSIZE) < PADDR(boot_alloc(0)))) {
			pages[i].pp_ref = 1;
