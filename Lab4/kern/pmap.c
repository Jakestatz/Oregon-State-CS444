...
page_init(void)
{
	...
	size_t i;
	for (i = 0; i < npages; i++) {
		if ((i == 0) || i * PGSIZE == 0x7000 || ((i * PGSIZE) >= IOPHYSMEM && (i * PGSIZE) < EXTPHYSMEM) || ((i * PGSIZE) >= EXTPHYSMEM && (i * PGSIZE) < PADDR((char*)end)) || ((i * PGSIZE) >= PADDR((char*)end) && (i * PGSIZE) < PADDR(boot_alloc(0)))) {
			pages[i].pp_ref = 1;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (pde & PTE_P) {
		...
		struct PageInfo* pageTable = page_alloc(ALLOC_ZERO);
		if (pageTable == NULL) {
			return NULL;
		}
		pageTable->pp_ref += 1;
		pgdir[PDX(va)] = page2pa(pageTable) | PTE_P | PTE_U | PTE_W;
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	//
	// Your code here:
	physaddr_t pa_end = ROUNDUP((uintptr_t)pa + size, PGSIZE);
	physaddr_t pa_start = ROUNDDOWN((uintptr_t)pa, PGSIZE);
	uintptr_t pa_offset = pa & 0xfff;
	uintptr_t va_start = base, old_base = base;
	uintptr_t new_base = va_start + pa_end - pa_start;
	size_t map_size = pa_end - pa_start;
	if (va_start + map_size > MMIOLIM) panic("MMIO Overflow");
